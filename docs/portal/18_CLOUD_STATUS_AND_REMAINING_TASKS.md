...
- that next-step record is now also complete as a stateful staging backend slice: the derived Issue 149 through Issue 151 chain ended on a reviewed Lambda Function URL plus DynamoDB persistence path managed from OpenTofu staging infrastructure, with `portal-staging-deploy` run `23041594174` and `portal-sns-staging-review` run `23041628020` both succeeding
- the active staging SNS backend is now the Terraform-managed Lambda Function URL path, the DynamoDB timeline table is `multicloudproject-portal-sns-staging-timeline`, and the earlier failed App Runner diagnostic path has been removed
- the fresh GitHub-tracked follow-up queue is now Issue 136 for the SNS production-hardening contract and Issue 137 for the March Azure planning-only batch; deeper child drafts remain unpublished until further explicit confirmation
- the first approved SNS production child split is now GitHub Issue 138 for the pre-deploy promotion gate recorded in docs/portal/issues/issue-154-sns-production-promotion-execution.md; later public-verification and rollback child drafts remain unpublished
