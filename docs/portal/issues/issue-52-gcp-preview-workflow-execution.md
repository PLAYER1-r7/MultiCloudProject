...
- Issue 53 側の live infra apply は完了し、`preview.gcp.ashnova.jp` は `34.128.181.172` へ解決、managed certificate も `ACTIVE` になったため、`docs/portal/issues/issue-53-gcp-preview-delivery-resource-execution.md` を `ready` として positive-path deploy validation を開始できる状態へ遷移した
- live positive-path validation として `portal-gcp-preview-deploy` run `22850625525` は build provenance check、GCS sync、deployment record artifact、post-deploy smoke checks をすべて通過して success で完了した
- run `22850350743` と `22850451691` の途中失敗から、GCP backend bucket preview path では root 解決用の bucket website metadata と known SPA routes 向け URL map rewrite が必要であり、deep-link path は SPA shell を返す 404 fallback を取りうるため smoke check は non-root route に限り shell markers を満たす 404 を許容するよう補正した
