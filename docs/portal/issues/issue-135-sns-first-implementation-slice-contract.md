...
Acceptance Criteria
- [ ] AC-1: first implementation slice の minimum deliverable が app behavior 単位で明文化されている
- [ ] AC-2: frontend、service、config、validation の workstream split と dependency order が読み取れる
- [ ] AC-3: completion signal と fail conditions が staging-oriented evidence path と結び付いている
- [ ] AC-4: first slice に含めない follow-on work が non-goals として切り分けられている
...
- [ ] first slice deliverable を fixed judgment にする
- [ ] workstream split and dependency order を fixed judgment にする
- [ ] completion signal と fail condition を fixed judgment にする
- [ ] validation and evidence path を fixed judgment にする
- [ ] first slice non-goals を明文化する

# Definition of Done

- [ ] guest timeline read と member post-readback の minimum path が読める
- [ ] frontend、service、config、validation の dependency order が読める
- [ ] fail-closed auth/error behavior が completion signal に含まれている
- [ ] staging-oriented evidence path が downstream execution issue で参照できる
- [ ] moderation UI、deep ops automation、multi-cloud write が first slice から外れている
...
- turn the planning chain into one executable first slice contract
