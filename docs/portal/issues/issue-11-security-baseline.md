...
受け入れ条件
- [x] 条件 1: 初回リリースで必須とする security baseline が control ごとに整理され、Issue 10・12・14 の判断基準として参照できる状態になっている
...
- [x] シークレット管理方針を定義する
- [x] 監査ログ方針を定義する
- [x] セキュリティチェックリストを作成する

## Definition of Done

- [x] HTTPS 強制の方針と適用箇所が明示されている
- [x] CORS の適用範囲と最小化方針が説明されている
- [x] セキュリティヘッダの基本方針が整理されている
- [x] シークレットをリポジトリとフロントエンドから排除する方針が明記されている
- [x] 監査や変更履歴の追跡方針が説明されている
- [x] WAF の要否判断が明示されている
- [x] 初期実装で外してはいけない項目のチェック観点として参照できる
...
For Issue 11 final review, the local issue record is the primary evidence source. [docs/portal/13_SECURITY_BASELINE_DRAFT.md](docs/portal/13_SECURITY_BASELINE_DRAFT.md) is supporting background and synchronized policy text, not the sole proof of checklist completion.

### Task Mapping

| Checklist item                         | Primary evidence section                                                                                                                                                                        | Why this is the evidence                                                                                                                                                                                       | Review state              |
| -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- |
| `HTTPS 強制方針を定義する`             | `3. HTTPS 強制方針のたたき台`, `Resolution`, and [docs/portal/13_SECURITY_BASELINE_DRAFT.md](docs/portal/13_SECURITY_BASELINE_DRAFT.md)                                                         | These sections define HTTPS as the required public path, describe redirect behavior, and make missing enforcement a blocker.                                                                                   | Accepted for final review |
| `CORS 制御方針を定義する`              | `4. CORS 制御方針のたたき台`, `Resolution`, and [docs/portal/13_SECURITY_BASELINE_DRAFT.md](docs/portal/13_SECURITY_BASELINE_DRAFT.md)                                                          | These sections define minimal exposure, prohibit speculative wildcard usage, and reserve broader CORS only for future API-backed interactions.                                                                 | Accepted for final review |
| `セキュリティヘッダ方針を定義する`     | `5. Security Header 方針のたたき台`, `11. この場で確認したい論点`, and `Resolution` in [docs/portal/issues/issue-11-security-baseline.md](docs/portal/issues/issue-11-security-baseline.md)     | These sections define the baseline header set, delivery-layer responsibility, and how staging and production parity should be handled.                                                                         | Accepted for final review |
| `WAF の要否を整理する`                 | `6. WAF 要否のたたき台`, `11. この場で確認したい論点`, and `Resolution` in [docs/portal/issues/issue-11-security-baseline.md](docs/portal/issues/issue-11-security-baseline.md)                 | These sections record that WAF is not mandatory for the first release, while still requiring explicit reasoning and reevaluation triggers.                                                                     | Accepted for final review |
| `シークレット管理方針を定義する`       | `7. Secret 管理方針のたたき台`, `10. Issue 10 / 12 / 14 への接続観点`, and `Resolution` in [docs/portal/issues/issue-11-security-baseline.md](docs/portal/issues/issue-11-security-baseline.md) | These sections define GitHub Environment as the CI/CD secret path, separate staging and production secret boundaries, reserve managed stores for runtime needs, and prohibit repository or frontend embedding. | Accepted for final review |
| `監査ログ方針を定義する`               | `8. 監査ログ方針のたたき台`, `11. この場で確認したい論点`, and `Resolution` in [docs/portal/issues/issue-11-security-baseline.md](docs/portal/issues/issue-11-security-baseline.md)             | These sections define the minimum audit evidence set and preserve audit logging as a non-optional security capability.                                                                                         | Accepted for final review |
| `セキュリティチェックリストを作成する` | `9. first-release security checklist のたたき台` and `Resolution` in [docs/portal/issues/issue-11-security-baseline.md](docs/portal/issues/issue-11-security-baseline.md)                       | These sections collect the required first-release controls into a reviewable checklist for implementation and release readiness.                                                                               | Accepted for final review |

### Definition Of Done Mapping

| Checklist item                                                             | Primary evidence section                                                                                                                                                                                          | Why this is the evidence                                                                                                                                                    | Review state              |
| -------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- |
| `HTTPS 強制の方針と適用箇所が明示されている`                               | `3. HTTPS 強制方針のたたき台`, `Resolution`, and [docs/portal/13_SECURITY_BASELINE_DRAFT.md](docs/portal/13_SECURITY_BASELINE_DRAFT.md)                                                                           | These sections define HTTPS redirect behavior and place it on the delivery path for both environments.                                                                      | Accepted for final review |
| `CORS の適用範囲と最小化方針が説明されている`                              | `4. CORS 制御方針のたたき台` and `Resolution` in [docs/portal/issues/issue-11-security-baseline.md](docs/portal/issues/issue-11-security-baseline.md)                                                             | These sections describe minimal exposure and explicitly reject permissive wildcard defaults.                                                                                | Accepted for final review |
| `セキュリティヘッダの基本方針が整理されている`                             | `5. Security Header 方針のたたき台`, `11. この場で確認したい論点`, and `Resolution` in [docs/portal/issues/issue-11-security-baseline.md](docs/portal/issues/issue-11-security-baseline.md)                       | These sections define the baseline header set and how parity and support gaps should be handled.                                                                            | Accepted for final review |
| `シークレットをリポジトリとフロントエンドから排除する方針が明記されている` | `7. Secret 管理方針のたたき台`, `Resolution`, and [docs/portal/13_SECURITY_BASELINE_DRAFT.md](docs/portal/13_SECURITY_BASELINE_DRAFT.md)                                                                          | These sections prohibit repository and frontend embedding while defining the approved control path for CI/CD secrets and separate staging and production secret boundaries. | Accepted for final review |
| `監査や変更履歴の追跡方針が説明されている`                                 | `8. 監査ログ方針のたたき台`, `Resolution`, and [docs/portal/13_SECURITY_BASELINE_DRAFT.md](docs/portal/13_SECURITY_BASELINE_DRAFT.md)                                                                             | These sections define the required evidence sources and preserve traceability for deploy and infra changes.                                                                 | Accepted for final review |
| `WAF の要否判断が明示されている`                                           | `6. WAF 要否のたたき台`, `11. この場で確認したい論点`, and `Resolution` in [docs/portal/issues/issue-11-security-baseline.md](docs/portal/issues/issue-11-security-baseline.md)                                   | These sections explicitly record the first-release decision, non-adoption rationale, and reevaluation triggers.                                                             | Accepted for final review |
| `初期実装で外してはいけない項目のチェック観点として参照できる`             | `9. first-release security checklist のたたき台`, `10. Issue 10 / 12 / 14 への接続観点`, and `Resolution` in [docs/portal/issues/issue-11-security-baseline.md](docs/portal/issues/issue-11-security-baseline.md) | These sections organize the baseline into a reusable checklist and connect it to delivery, monitoring, and rollback work.                                                   | Accepted for final review |

## Final Review Result

Final checkbox review completed against the latest wording in [docs/portal/issues/issue-11-security-baseline.md](docs/portal/issues/issue-11-security-baseline.md), with [docs/portal/13_SECURITY_BASELINE_DRAFT.md](docs/portal/13_SECURITY_BASELINE_DRAFT.md) used as synchronized supporting policy text. The table below records the document-level validation judgment. Human close approval remains a separate action.

| Checklist area                    | Final judgment | Evidence basis                                                                                                                                                                                                                                                                        |
| --------------------------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| HTTPS baseline                    | Satisfied      | `3. HTTPS 強制方針のたたき台`, `Resolution`, and [docs/portal/13_SECURITY_BASELINE_DRAFT.md](docs/portal/13_SECURITY_BASELINE_DRAFT.md) confirm HTTPS redirect and release-blocking treatment for missing enforcement.                                                                |
| Minimal CORS exposure             | Satisfied      | `4. CORS 制御方針のたたき台` and `Resolution` confirm minimal exposure and no wildcard production posture.                                                                                                                                                                            |
| Security header baseline          | Satisfied      | `5. Security Header 方針のたたき台`, `11. この場で確認したい論点`, and `Resolution` confirm the baseline header set and parity rule.                                                                                                                                                  |
| Secret posture                    | Satisfied      | `7. Secret 管理方針のたたき台`, `Resolution`, and [docs/portal/13_SECURITY_BASELINE_DRAFT.md](docs/portal/13_SECURITY_BASELINE_DRAFT.md) confirm approved secret paths, separate staging and production CI/CD secret boundaries, and prohibition on repository or frontend embedding. |
| Auditability                      | Satisfied      | `8. 監査ログ方針のたたき台`, `Resolution`, and [docs/portal/13_SECURITY_BASELINE_DRAFT.md](docs/portal/13_SECURITY_BASELINE_DRAFT.md) confirm traceable deploy, infra, approval, and operator steps.                                                                                  |
| WAF decision                      | Satisfied      | `6. WAF 要否のたたき台`, `11. この場で確認したい論点`, and `Resolution` confirm that WAF is documented as not mandatory now, with reevaluation triggers retained.                                                                                                                     |
| First-release checklist readiness | Satisfied      | `9. first-release security checklist のたたき台`, `10. Issue 10 / 12 / 14 への接続観点`, and `Resolution` confirm that the baseline can be reused by downstream issues.                                                                                                               |

## Current Status

- [docs/portal/13_SECURITY_BASELINE_DRAFT.md](docs/portal/13_SECURITY_BASELINE_DRAFT.md) を security judgment の同期先として扱う
- local issue file には task contract、discussion draft、resolution、evidence mapping、final review result を追加済み
- HTTPS redirect、baseline headers、minimal CORS、managed secrets、auditability を first-release 必須 control として確定した
- WAF は first-release 必須要件にはせず、非採用理由と再評価条件を残す方針で確定した
- CI/CD secret の正規経路は approved GitHub Environment とし、staging と production の secret boundary は別 environment で分離する方針で確定した
- runtime secret は必要時に managed store へ分離し、secret source ごとに rotation path を持つ方針で確定した
- staging と production の header baseline は揃えることを目標とし、support gap は記録するが production を弱めない方針で確定した
- first-release の最低 audit evidence は GitHub workflow history と cloud-native audit log の両方を参照可能にする方針で確定した
