...
- route baseline と staging verification posture から `PRODUCTION_SMOKE_PATHS=/,/overview,/guidance` は確定できるため GitHub environment variable へ投入済みである。一方、approved production custom-domain の具体 URL は repo と AWS 実体からは特定できず、`PRODUCTION_BASE_URL` は cutover owner が確定値を入力する follow-up として残る
...
- FOLLOW-UP: `PRODUCTION_SMOKE_PATHS` は `/,/overview,/guidance` として GitHub environment へ投入済みである一方、approved production custom-domain の具体 URL は repo / AWS 実体からは確認できず、`PRODUCTION_BASE_URL` は cutover owner が確定値を入力する必要がある
...
- route baseline と staging verification posture から `PRODUCTION_SMOKE_PATHS=/,/overview,/guidance` は確定できたため GitHub environment variable へ投入済みである。staging では `STAGING_BASE_URL=https://d32v64hg1mmmau.cloudfront.net` が使われているが、production の approved custom-domain 文字列および alias はまだ repo / AWS 実体に現れていないため、`PRODUCTION_BASE_URL` は owner-managed follow-up として残す
...
- implementation sync と formal review は完了しており、close 判定と `PRODUCTION_BASE_URL` の確定投入は未実施である
