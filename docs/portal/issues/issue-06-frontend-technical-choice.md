...
- static-first recommendation は draft 上に存在するが、Issue 6 としてはまだ比較結果の明文化と採用理由の確定が必要である
...
- local issue file には discussion starter と review 観点を追加し、Issue 6 の議論開始点を揃えた
- static-first を第一候補とする暫定合意案を追加した
- static site / SPA / SSR の比較結果と adoption decision を追加した
- checkbox 根拠の対応付けとして Resolution と Evidence Mapping Table を追加した
- 採用案は static-first TypeScript frontend with Vite として確定した
- routing、build 方式、environment variable policy は first-release decision として文書上で整列済みであり、final checkbox review の確認対象に入る
