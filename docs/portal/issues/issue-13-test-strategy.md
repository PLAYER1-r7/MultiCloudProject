...
- この Issue は現時点では planning / draft 段階のままであり、Resolution の追加は test baseline の方針合意を記録するものであって、Tasks や Definition of Done の完了、Issue close 承認、Final Review Result を意味しない。

## Current Status

- local issue file には first-release test baseline の discussion draft と Resolution を記録済み
- Section 10 の open questions は Resolution 確定文言列で final wording と対応付けた
- smoke test、major flow check、module-level test、post-deploy verification、staging acceptance の境界は Issue 18 実装が参照できる粒度で固定した
