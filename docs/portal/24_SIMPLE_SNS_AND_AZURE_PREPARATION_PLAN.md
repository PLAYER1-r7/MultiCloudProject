...
- fix the minimum monitoring signals, rollback trigger line, critical-path test set, and post-rollback evidence required for the first stateful SNS release
...
- fix ownership boundary、shared-layer gate、regression target、and explicit exception process before implementation starts
...
7. Stateful monitoring, rollback, and test baseline
   内容: Issue 12、Issue 13、Issue 14 の static-first baseline を拡張し、auth/API/database を含む failure mode、critical-path test set、rollback trigger、post-rollback evidence を fixed judgment にする。
...
8. SNS implementation protection and change-isolation baseline
   内容: unrelated feature が SNS の route、contract、service、shared dependency を壊さないために、module ownership、shared-layer touch rule、critical-path regression gate、explicit exception process、shared-layer compatibility check を fixed judgment にする。
...
- implement SNS on a single-cloud execution boundary first, with guest timeline read on the service-backed read path, guest write rejected fail-closed by the service contract, member valid post on the service-backed write path, invalid payload rejected through the stable error contract, and post-readback consistency observed without local-only fake state dependence
- keep frontend route and app contracts cloud-neutral
- postpone Azure actual implementation and live resource spend
- complete auth/API/data/monitoring/test/rollback for one cloud in fixed dependency order: stable service contract and config path, minimum persistence-backed read/write path, frontend integration, then validation and staging-oriented evidence
