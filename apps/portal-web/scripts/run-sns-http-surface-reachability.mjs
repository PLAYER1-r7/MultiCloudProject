...
const previewHost = process.env.SNS_HTTP_SURFACE_PREVIEW_HOST ?? "127.0.0.1";
const serviceHost = process.env.SNS_HTTP_SURFACE_SERVICE_HOST ?? "127.0.0.1";
...
const tempDirectory = await mkdtemp(join(tmpdir(), "portal-web-sns-http-surface-"));
const storageFilePath = join(tempDirectory, "sns-service-timeline.json");
const previewPort = Number(process.env.SNS_HTTP_SURFACE_PREVIEW_PORT ?? (await getAvailablePort(previewHost)));
const servicePort = Number(process.env.SNS_HTTP_SURFACE_SERVICE_PORT ?? (await getAvailablePort(serviceHost)));
  ...
  const targets = [
    {
      ...
      expectedRuntimeStatusIncludes: "HTTP service mode is configured against",
      expectedServiceBaseUrl: serviceBaseUrl
    }
  ];
