...
- 対象アプリ、環境、目的を特定する。
- 明示的な制約と暗黙のリスクを抽出する。
- 独立に確認できる参照が複数ある場合は、まず read-only の文脈収集を並列で行う。
- 並列の read-only 文脈収集後は、現在の理解と次の行動を短く進捗共有する。
- ユーザーが「残タスク」「次タスク」「何が残っているか」などを明示的なスコープなしで求めた場合は、全体プロジェクトへ拡張せず、直近の active task contract、issue chain、または current app context にデフォルトで係留する。
- もっともらしいスコープ候補が 2 つ以上ある場合は推測しない。候補スコープを明示し、どれを指すか確認してから project-wide な残タスク一覧を出す。
- 現在の active scope からリポジトリ全体の task inventory へ広げる前には、「project-wide」「リポジトリ全体で」など、明示的な全体化指示を必須とする。
- 曖昧な task-list 要求への最初の応答では、エージェントが今どのスコープで解釈しているかを明記し、ユーザーが即座に修正できるようにする。
- 要求が child issue または follow-up issue chain の新規作成や継続につながる可能性がある場合は、次の Issue が新しい証拠採取、新しい固定判断、新しい実行境界のいずれかを追加するか確認する。
- 提案中の次 Issue が packaging-only な言い換えや再記述に留まる場合は、受領を停止し、作業を現行 Issue に残す。
- 同じ chain で連続する 2 件の Issue が、新しい証拠、新しい固定判断、新しい実行境界のいずれも増やしていない場合は、分解を停止し、その chain を close すべきか見直す。
- 提案中の次 Issue が同じ chain の 4 件目になる場合は、受領前に明示的な人間確認を必須とする。
...
- 小さく、巻き戻せる変更で進める。
- 他者編集、formatter、再開セッション、またはツール警告で変更の可能性があるファイルは、編集直前に現行内容を読み直す。
...
- 残差分が意味変更と formatting-only cleanup に分かれる場合は、review 根拠と close 承認の対象範囲を正確に保つため、別タスク契約または別 issue に切り分ける。
- 無関係な編集を混ぜない。
- 長めの実装パスでは、計画全文の繰り返しではなく、差分だけを短く進捗共有する。
...
- 関連検証を実行できなかった場合は、その欠落を task record または handoff に明記する。

### 5. 取りまとめ

- 変更内容、検証結果、残存リスクを要約する。
- レビュー完了を宣言する前に、checkbox 状態、status セクション、リモート Issue 状態を揃える。
- リモートの Issue / PR 番号対応、status wording、または公開本文がローカル正本とずれている場合は、先にローカル記録を補正して読み直し、そのリポジトリ内ファイルを次回同期の source of truth として扱う。
...
- 最後の review pass、承認やり取り、またはツール通知の後に根拠文書が変わっている可能性がある場合は、Process Review Notes、close 承認記録、その他の最終状態向け編集を加える前に現行ファイルを読み直し、formatter や人手の編集を壊さないようにする。
...
- ネットワーク失敗やリモート不安定性で複数 Issue の一括同期が不安定になった場合は、一括同期を止めて 1 issue ずつへ切り替え、各 retry の対象を単一化する。
...
- PR タスク契約テンプレートを使う。
- 未解決点とレビュー重点を明示する。
- 実装や検証がまだ必要な段階で部分分析のまま終了せず、依頼範囲が完了するか本物の blocker を文書化した時点でのみ引き継ぐ。
- 1 issue ずつの remote retry へ切り替えた場合は、その方式で処理した issue 番号を handoff の Evidence または Actions taken に明記する。
