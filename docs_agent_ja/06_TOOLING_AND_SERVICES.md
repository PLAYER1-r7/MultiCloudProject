...
- 推測で設定値を埋めない
- テスト、コマンド、ランタイム結果は、このセッションで観測したものか、特定可能な artifact または handoff record を出所として明示できる場合にだけ current fact として扱う
- まだ未検証の内容は仮説として明示し、何を確認すれば確定するかを書く
- 既存スクリプトを優先して使う
- 手動操作は最小限にする
...
- エージェントは差分確認、スコープ要約、PR タイトルと本文の作成、branch 作成、意図したファイルだけの stage、commit、push、PR 起票まで進めてよい。
- エージェントは self-review コメント、検証コメント、merge 前チェックリスト、follow-up issue 候補の作成も支援してよい。
- 最終的なスコープ承認、merge 判断、レビュー指摘の受け入れ判断、本番リスク判断、検証証跡が十分かどうかの判断は人間が持つ。
- スコープが明確で、破壊的操作を伴わない場合は、エージェントは PR 作成まで自律的に進めてよい。
- スコープが曖昧な場合、無関係な差分が混在する場合、merge 安全性が判断依存になる場合は、エージェントは止まって確認を求める。
...
- 現在の workspace branch が別作業に使われている場合は、無関係な差分を混ぜないため、PR コメント対応は `/tmp/` 配下の分離 worktree などで行うことを優先する。
...
- PR が issue 記録や status tracking docs も更新している場合は、再レビュー依頼前に predecessor 表現、closed/open 表現、status summary が現在の GitHub 状態と一致しているかを再確認する。
