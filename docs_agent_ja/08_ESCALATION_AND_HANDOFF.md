...
- 関連検証を実行していない場合は、完了を匂わせず `Evidence` または `Risks or blockers` に明示する
- 実装や検証がまだ必要で本物の blocker もない場合は、部分分析の段階で `Handoff ready` を使わない
...
- read-only の文脈収集や再読がエスカレーション判断に効いた場合は、その要点を `Actions taken` に短く書く
- エスカレーション前に関連検証を実行していない場合は、その事実を `Evidence` に明記する
...
- 検証証跡とロールバック証拠は `Evidence` に入れ、未実行の関連確認があれば併記する
- remote の issue 作成や本文同期が不安定な場合は、1 issue ずつ処理し、扱った issue 番号を `Actions taken` または `Evidence` に明記する
...
```text
Document: 08_ESCALATION_AND_HANDOFF
Scope: single-issue retry 運用へ切り替えた後の MultiCloudProject SNS / Azure issue chain 正規化と GitHub 本文再同期 checkpoint
Outcome: Handoff ready
Actions taken: 差し替え後の AI-agent handoff guide を再読し、正規 execution-record 形式は崩さずに今回の到達点を整理した; SNS / Azure のローカル issue record である Issue 135 から 156 を fixed judgment または fixed execution 形式へ正規化し、checked acceptance criteria、process review notes、current-status 行をそろえた; GitHub issue 作成後にローカル番号とのずれが見つかったため、対応する GitHub 番号をローカル正本へ反映した; 承認済み production child split を publish し、GitHub Issue 136 の子として Issue 138、152、151 を順に接続した; その後、open な GitHub issue 本文に残っていた stale な local-only wording をローカル正本へ合わせて再同期し、ネットワークエラーが頻発した段階からは 1 issue ずつの単体更新へ切り替えて残件を個別に完了させた
Evidence: ローカル正本では SNS / Azure follow-up issue の GitHub 番号対応が 136 から 147 および 152 から 156 まで整合している; open な GitHub issue 本文は Issue 137、139 から 150、151、152 についてローカル正本の本文へ再同期済みである; GitHub 上では parent-child 構造として Issue 136 の配下に Issue 138、152、151 が記録されている; ローカル文書の正規化バッチでは markdown diagnostics が通過しており、その後の再同期は GitHub issue 本文だけを更新した
Risks or blockers: 今回の remote pass で open issue の presentation drift は解消したが、closed issue や今後 publish される follow-up issue に drift が出た場合は別途 audit が必要である; GitHub 側の一括更新は今後もネットワーク不安定性の影響を受けやすいため、失敗が再発する場合は 1 issue ずつの retry を継続する必要がある
Closure rationale: この checkpoint は、承認済み SNS / Azure issue chain のローカル正規化、publish 済み GitHub issue 対応の補正、production child relationship の復元、open GitHub body の正本回帰までを完了条件とし、新しい実装スコープへ広げず presentation と chain integrity の回復で止めた
Next action: この issue chain では docs/portal/issues 配下のローカル記録を source of truth として扱い、将来 GitHub issue audit を再開する場合もまずそこから差分確認を始める; ネットワーク不安定時の remote 再同期は batch ではなく 1 issue ずつで進める
```
