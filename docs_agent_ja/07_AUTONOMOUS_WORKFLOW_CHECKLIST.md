...
- [ ] スコープと受け入れ条件を固定した
- [ ] 必要な認証を確認した
- [ ] 独立に確認できる参照が複数ある場合、read-only の文脈収集を並列化した
- [ ] 並列の read-only 文脈収集後に、短い進捗共有と次の一手を明示した
- [ ] ユーザー編集、formatter、セッション再開、またはツール警告でファイル変更の可能性がある場合、編集直前に現行内容を読み直した
...
- [ ] 境界チェックがクリーンである
- [ ] ロールバック経路が維持されている
- [ ] 長い作業では、計画全文ではなく差分だけを短く進捗共有した
- [ ] 本物の blocker がない限り、部分分析で止めず実装と検証まで進めた
...
- [ ] PR タイトル、本文、self-review メモを AI 支援で準備した場合は、最終提出または merge 前に人間がスコープ、検証証跡、merge リスクを確認する必要があることを PR 文面で明示し、実際に確認される前は確認済みと書かない
...
- [ ] テスト結果を記録した
- [ ] DoD の必須ゲートを満たした
- [ ] 関連検証を実行できなかった場合は、その制約と理由を明示した
- [ ] Issue close または引き継ぎの流れに、無関係な未コミット変更を混ぜていない
- [ ] リモートの Issue / PR 状態がローカル番号対応や status wording とずれていた場合は、先にローカル正本を補正し、その後の同期ではそのローカル記録を source of truth として扱った
...
- [ ] ネットワーク失敗やリモート不安定性で複数 Issue の一括同期が不安定になった場合は、残りの同期を 1 issue ずつへ切り替え、処理した issue 番号を handoff evidence に記録した
