...
- Issue 分解ガードレール、create gate、`terminal condition` 項目を片言語で追加または変更した場合は、同一タスク内でもう片方にも同じ構造変更を反映する
- 残差分に意味変更と formatting-only cleanup が混在する場合は、最終 review の根拠が曖昧にならないよう別スコープとして記録・追跡する
...
- formatter、別エディタ、またはセッション再開によって最新の review pass 後にローカル根拠文書が変わっている可能性がある場合は、承認記録やその他の最終状態向け編集を加える前に現行ファイルを読み直す
...
- タスクが Issue chain を停止、close、または child issue 拒否に切り替えるルールを変更する場合は、完了前に `01`、`02`、`03`、`07`、`08`、`09`、`14` の英日両方で同じ stop-condition 文言が保たれていることを確認する
...
- [ ] 章構成が一致している
- [ ] 文書意図が一致している
- [ ] Issue 分解の stop-condition 文言が、対応する英語文書と日本語文書で一致している
