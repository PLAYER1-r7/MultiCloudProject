...
- The current documented snapshot supports a single-approver model, an approved externally owned custom-domain path, and external DNS coordination, while artifact retention and rollback evidence now exist in the staging-first workflow path, the monthly cost ceiling is fixed at USD 15/month before tax, and native S3 state locking is enabled in both the staging and production backend configurations
